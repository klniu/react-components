...
class LoginAndPasswordForm extends React.Component<LoginAndPasswordFormProps, void> {
    ...
    checkPass = (_rule: {}, value: string, callback: Function) => {
        if (value.length < 6) {
            callback('密码长度过短');
        }
        callback();
    ...
    checkPass2 = (_rule: {}, value: string, callback: Function) => {
    ...
    render() {
        ...
        const newPassItem1 = (
            <Form.Item key="newPass1">
                {getFieldDecorator('newPass1', {
                    rules: [{required: true, message: '请输入新密码!'}, {validator: this.checkPass}],
                })(
                    <Input
                        prefix={<Icon type="lock"/>}
                        type="password"
        ...
        const newPassItem2 = (
            <Form.Item key="newPass2">
                {getFieldDecorator('newPass2', {
                    rules: [{required: true, message: '请再次输入新密码!'}, {validator: this.checkPass2}],
                })(
                    <Input
                        prefix={<Icon type="lock"/>}
                        type="password"
        ...
        var components: JSX.Element[] = [userNameItem, passwordItem, loginItem];
        if (this.props.operate === 'changePass') {
            components = [originPasswordItem, newPassItem1, newPassItem2, changePassSubmit];
        }
        return (
